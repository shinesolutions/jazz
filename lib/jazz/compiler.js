...
Compiler_JS.prototype._prologue = function(symtable) {
  var code =  "";
  code += "(function() {\n";
  ...
  // XXX don't think we need a new instance for every call ...
  ...
  for (var i = 0; i < symtable.length; i++) {
    var j = i + 1;
    code += "var " + symtable[i] + " = arguments[" + j + "];\n";
  }
  return code;
}
...
Compiler_JS.prototype.compile = function(suite, symtable) {
  var code = this._prologue(symtable);
