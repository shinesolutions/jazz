...
Compiler_JS.prototype._prelude = function(symtable) {
  ...
  code += "var __jazz_compiled = false;\n";
  code += "__jazz_emitter.addListener('data', function(chunk) {\n";
  code += "  if (--__jazz_expected < 0 && __jazz_compiled) {\n";
...
Compiler_JS.prototype._enterOutputScope = function(slotVar) {
  var code = "";
...
Compiler_JS.prototype._leaveOutputScope = function() {
  return "})();\n";
}
...
Compiler_JS.prototype._compileCall = function(stmt) {
  ...
  code += this._enterOutputScope(slotVar);
  code += stmt.id.name + "(function(data) { __jazz_emitter.emit('data', " + slotVar + ", data); });\n";
  code += this._leaveOutputScope();
