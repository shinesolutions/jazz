...
Compiler_JS.prototype._compileStatement = function(stmt) {
  ...
  else if (stmt.constructor == ast.WhenStmt) {
    code += this._compileWhenStmt(stmt);
  }
...
Compiler_JS.prototype._compileWhenStmt = function(stmt) {
    var code = "";
	
    code += "(" + this._compileExpr(stmt.expr.expr) + ")(";
    for (var i = 0; i < stmt.expr.args.length; i++) {
      code += this._compileExpr(stmt.expr.args[i]);
      code += ", ";
    }
    code += "function(conditionCheckResult) { ";
	if(stmt.expr.negateResult)
	{
		code += "if(!(conditionCheckResult == true))";
	}
	else
	{
		code += "if(conditionCheckResult == true)";
	...
	code += this._compileSuite(stmt.suite);
    if (stmt.otherwise) {
      code += " else " + this._compileStatement(stmt.otherwise);
    }
	code +=  "});\n";
