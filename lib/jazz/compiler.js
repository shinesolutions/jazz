...
Compiler_JS.prototype._prelude = function(symtable) {
  ...
  code += "    process.nextTick(function() { __jazz_emitter.emit('end'); });\n";
  code += "  }\n";
  code += "});\n";
...
Compiler_JS.prototype._nextPos = function() {
  return this.nextPos++;
}
...
Compiler_JS.prototype._compileStatement = function(stmt) {
  ...
  if (stmt.constructor == ast.IfStmt) {
    ...
    code += "__jazz_expected++;\n";
    code += "process.nextTick(function() {\n";
    code += "__jazz_emitter.emit('data', " + this._nextPos() + ", " + this._compileExpression(stmt.value) + ");\n";
    code += "});\n";
  }
  else if (stmt.constructor == ast.Call) {
    code += "__jazz_expected++\n";
    code += "process.nextTick(function() {\n";
    code += expr.id.name + "(function(data) { __jazz_emitter.emit('data', " + this._nextPos() + ", data); })\n";
    code += "});\n";
  }
  else {
