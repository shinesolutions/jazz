...
Parser.prototype._parseWhenStmt = function() {
  this._expect(tokens.WHEN);
  var negateResult = false
  if(this.current().type == tokens.NOT)
  {
  	this._expect(tokens.NOT);
	negateResult = true;
  }
  var expr = this._parseEchoExpr();
  expr.negateResult = negateResult;
  var suite = this._parseSuite();
  var root = new ast.WhenStmt(expr, suite);
  var ref = root;
  if (this.current().type === tokens.OTHERWISE) {
    this._expect(tokens.OTHERWISE);
    ref.otherwise = this._parseSuite();
  }
  this._expect(tokens.END);
  return root;
}
