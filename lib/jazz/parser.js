...
Parser.prototype._syntaxError = function(tok, message) {
  throw new scanner.SyntaxError(this.scanner.filename, tok.row, tok.col, message);
}
...
Parser.prototype._parseExpr = function() {
  // currently we only support simple idents-in-expressions
  
  var result = this._parseIdent();
  ...
  if (this.current().type == '(') {
    this.next();
    this._expect(')');
    return new ast.Call(result);
  } else {
    return this._parseGetAttrList(result);
  }
}
...
Parser.prototype._parseSuite = function() {
  ...
  while (more) {
    ...
    switch (c.type) {
      ...
      case tokens.IDENT:
        {
          body.push(this._parseEchoIdent());
          break;
        }
      case tokens.FOREACH:
        {
...
Parser.prototype._parseEcho = function() {
  ...
  this.next();
  return new ast.Echo(value)
}

Parser.prototype._parseEchoIdent = function() {
  var result = this._parseIdent();
  if (!this._isLocal(result.name)) {
    this._declareGlobal(result.name);
  }
  if (this.current().type == '(') {
    this.next();
    this._expect(')');
    return new ast.Echo(new ast.Call(result));
  } else {
